...
class FlowGraphGenerator(DBConnect):
    ...
    def model_to_am(self):
        """ Function to generate the edge from model to am. The dsar created by the AM contains a DSIR-LIST. Each DSIR
        contains a field 'parent' which points to the DSIRS from which it got sampled and aligned. Using one of this
        DSIR, find its DSAR, which should have been created by the window manager. Now perform another back-hop to move
        backward in the provenance to find the DSAR created by the PSM. These DSAR form the MODEL node, and the
        corresponding initial DSAR used form the AM node
        """
        try:
            self.connect_db()
        ...
        try:
            ...
            for dsar in dsar_list:
                ...
                for dsir_id in dsir_list_id:
                    ...
                    # creating a dummy relationship from job to cluster_aggr if there is neither clustering nor
                    # aggregation
