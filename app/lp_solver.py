...
class LpSolver:
    ...
    def generate_diversity_hamming_constraints(self):
        """Function to generate constraints for diverse paths by modelling a cost incurred, to select the previously
        selected path. Cost is modelled as sum(1-Ep) <= N where Ep is the edges in the path and N is a numerical constant
        """
        path = []
        diverse_const = None
        for v in self.MODEL.getVars():
            if v.x == 1:
                path.append(v.varName)
                print(v.varName)
                if diverse_const is None:
                    diverse_const = 1 - self.MODEL.getVarByName(v.varName)
                else:
                    diverse_const += 1 - self.MODEL.getVarByName(v.varName)

        self.MODEL.addConstr(diverse_const == 2, "d0")
        return
    ...
    def solve_diversity_hamming_path(self):
        """Function to solve optimal path under the hamming constraints."""
    ...
    def solve_diversity_heuristic_path(self):
        """Function to solve optimal path under the heuristic objective function"""
        # solving for the diverse_heuristic_path
        self.MODEL.optimize()
    ...
    def solve_diversity_heuristic_2_path(self):
        """Function to solve optimal path under the heuristic_2 objective function"""
        # solving for the diverse_heuristic_path
        self.MODEL.optimize()
