...
class ModelGraph:
    ...
    def create_node(self, dsir):
        """
        Function to convert the DSIR to a node for the execution_provenance_graph
            :parameter
    ...
    def generate_edge(self, dsir, node, store):
        ...
        try:
            ...
            # Base Case
            if "children" not in dsir or len(dsir["children"]) == 0:
                if store:
                    if dsir_model_info["psm_settings"]["psm_strategy"] == "cluster":
                        self.update_node(dsir["_id"], "intermediate", dsir["metadata"]["model_type"], "", "")
                    else:
            ...
            if dsir["created_by"] == "JobGateway":
                for child_id in dsir["children"]:
                    ...
                    elif dsir_child["created_by"] == "AM_resampling":
                ...
                for child_id in dsir["children"]:
                    # This is a DSIR created by AlignmentManager (AM_resampling)
                    dsir_child = dsir_collection.find_one({"_id": child_id, "workflow_id": self.workflow_id})
                    # Move forward to find the descendant DSIR which is created by the JobGateway
                    dsir_descendant_id_list = dsir_child["children"]
                    for dsir_descendant_id in dsir_descendant_id_list:
    ...
    def generate_model_graph(self):
        ...
        try:
            dsir_collection = self.MONGO_CLIENT["ds_results"]["dsir"]
            store = False
            mg_workflow_collection = self.MONGO_CLIENT["model_graph"]["workflows"]
    ...
    def delete_graph(self):
        """
        Deleting the data in model_graph of a specific workflow
        """
        try:
            # removing all existing edge-node documents in edge_node collection in flow_graph_path database
            self.MONGO_CLIENT["model_graph"]["edge"].remove({"workflow_id": self.workflow_id})
            # removing all existing node-edge vertex pairs in flow_graph_path database
            self.MONGO_CLIENT['model_graph']['node'].remove({"workflow_id": self.workflow_id})
            # removing the workflow
            self.MONGO_CLIENT["model_graph"]["workflows"].remove({"workflow_id": self.workflow_id})
        except Exception as e:
            raise e

    def generate_window_number(self):
        """
        Function to generate window numbers for each simulation instance present as a node in node_collection. The window_num ranges [1, #execution_steps]
        """
        try:
            node_collection = self.MONGO_CLIENT["model_graph"]["node"]
            edge_collection = self.MONGO_CLIENT["model_graph"]["edge"]
            ...
            while len(start_nodes) > 0:
                ...
                for i in range(len(self.model_list)):
                    begin = model_window_start[i] + model_window_width[i] - model_window_shift[i]
                    ...
                    if len(dsir_id_list) > 0:
                        ...
                        model_window_start[i] += model_window_width[i] - model_window_shift[i]
